...
import {
  CreateAuthCodePostResponse,
  CreateAccessTokenPostResponse,
  GetUserSettingsPostResponse,
  GetAllDeviceDataPostResponse,
} from "./types";
...
export default class RControlAPI {
  ...
  login = async (username: string, password: string) => {
    const createAuthCodeResponse = await this.createAuthCode(
      username,
      password
    );
  ...
  createAuthCode = async (
    username: string,
    password: string
  ): Promise<CreateAuthCodePostResponse | undefined> => {
    ...
    try {
      ...
      console.error(
        "Failed to login to RControl; could not create authorization code. Error: " +
          error
      );
      return undefined;
    }
  ...
  createAccessToken = async (
    authCode: string
  ): Promise<CreateAccessTokenPostResponse | undefined> => {
    ...
    try {
      ...
      console.error(
        "Failed to login to RControl; could not create access token. Error: " +
          error
      );
      return undefined;
    }
  ...
  getUserSettings = async (): Promise<
    GetUserSettingsPostResponse | undefined
  > => {
    ...
    try {
      ...
      console.error(
        "Failed to fetch current alarm state; could not get user settings. Error: " +
          error
      );
      return undefined;
    }
  ...
  getAllDeviceData = async (body: {}): Promise<
    GetAllDeviceDataPostResponse | undefined
  > => {
    if (!this.isLoggedIn()) return;
    ...
    try {
      ...
      console.error(
        "Failed to fetch current alarm state; could not get device data. Error: " +
          error
      );
      return undefined;
    }
  ...
  updateArmStatus = async (body: {}) => {
    ...
    try {
      ...
      console.error("Failed to update alarm state. Error: " + error);
      return undefined;
    }
  };
}
